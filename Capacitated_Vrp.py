...
class Classic_VRP_Capacitated:
    ...
    def Vehicle_Mutation(self, Vehicle):
        ...
        Mutation_Size = int(max(np.ceil(self.Input.shape[0]/6), 3))
...
Vehicle_Number2 = 20
Vehicle_Capacity = np.array([11195 for Vehicle in range(Vehicle_Number2)])
np.random.seed(seed = 1)
City_Demand = np.random.randint(6,7,size = Sample_Number, dtype = int)
City_Demand[0] = 50
City_Demand[1] = 32
City_Demand[2] = 32
np.random.seed(seed = 1)
...
VRP_Object3 = Classic_VRP_Capacitated(Iteration = 500, Population_Size = 500, Vehicle_Num=Vehicle_Number2)
