...
# In[]:
class Hub_Loc:
    def __init__(self, Iteration = 1000, Hub_Number = 5, Hub_Distance_Coeff = 0.4):
    ...
    def Find_Objective(self, Hub_Solution, Hub_Assignment):
        ...
        for i in range(self.Input.shape[0]):
            for j in range(i+1, self.Input.shape[0]):
                Hub_connection_cost += self.Distance[i,Hub_Assignment[i]] + self.Hub_Distance_Coeff * self.Distance[Hub_Assignment[i], Hub_Assignment[j]] +  self.Distance[Hub_Assignment[j],j] 
        return Hub_connection_cost
    def Hub_location(self, Input, Population_Size = 50):
        self.Input=Input
        ...
        print(self.Find_Objective(self.Initial_Hubs[0],self.Initial_Assignment[0]))
...
Hub_Object = Hub_Loc(Iteration = 500, Hub_Number = Hub_Number , Hub_Distance_Coeff = 0.5)
Hub = Hub_Object.Hub_location(Locations, Population_Size = 50)
