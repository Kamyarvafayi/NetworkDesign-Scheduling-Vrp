...
class Staff_Shift_Scheduling:
    ...
    def Mutation(self, Solution):
        ...
        New_Solution[Row_Index,Column_Index] = np.random.randn()*0.1+New_Solution[Row_Index,Column_Index]
        if New_Solution[Row_Index,Column_Index]>1:
    ...
    def Find_Objective(self, Solution):
        ...
        return self.Wage*np.sum(Sol) + np.sum(Demand_Violation)*self.Demand_Violation_Cost 
...
for i in range(1, Shift_Number+1):
    for j in range(1, Staff_Number+1):
        if Final_Solution[i-1, j-1]==1:
            ...
            plt.text(j-0.5+length/2-0.25, i-0.5+width/2-0.1, int(j), fontsize = 8)
